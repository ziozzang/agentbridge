...
| `google-vertex`, `google-antigravity`, `anthropic-vertex` | Cloud/Vertex auth와 endpoint signing 필요. |
| `copilot-acp` | 외부 ACP process transport 필요. |
| `github-copilot` | Copilot token/catalog 처리 필요. |
| `amazon-bedrock`, `amazon-bedrock-mantle` | AWS SigV4와 Bedrock Converse transport 필요. |
| `minimax-portal`, `minimax-oauth` | API-key OpenAI 호환 MiniMax 외의 portal/OAuth flow 필요. |
| `byteplus`, `byteplus-plan`, `volcengine`, `volcengine-plan`, `modelstudio`, `qwencloud` | 기본 활성화 전 endpoint/auth 검증 필요. |
...
| `huggingface`, `novita`, `arcee`, `gmi`, `xiaomi`, `tencent-tokenhub`, `mistral`, `groq`, `fireworks`, `together`, `cerebras`, `chutes`, `deepinfra`, `moonshot`, `minimax`, `qwen`, `qianfan`, `venice`, `cloudflare-ai-gateway`, `microsoft-foundry`, `ollama-cloud`, `lmstudio`, `vllm`, `sglang` | provider별 API key 또는 local key | OpenAI Chat Completions 호환 | 내장 agent tool |
